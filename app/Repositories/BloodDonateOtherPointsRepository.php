...
class BloodDonateOtherPointsRepository implements BloodDonateOtherPointsRepositoryInterface
{
    protected BloodDonateOtherPoint $model;

    public function __construct(BloodDonateOtherPoint $model)
    {
        $this->model = $model;
    }
    ...
    public function getForDataTable(array $filters = []): Builder
    {
        ...
        // Date range filtering by request_date (Req. Date)
        ...
        if ($fromDate !== '' && $toDate !== '') {
            try {
                ...
                // ignore parse error, no filter applied
            }
        } elseif ($fromDate !== '') {
            try {
                $start = Carbon::parse($fromDate)->startOfDay();
