...
$('#submit_bmiss_form').click(function(e) {
      ...
      $(".page-loader").css({'position':'relative','top':'50%','left':'50%'});
        ...
        if(parseInt($("#exYear").val())>2 && ($("#project_title").val() == '' || ($("#project_title").val() == null))){            
        ...
        $.ajax({
            type: 'POST',
            url: $('#BmissSubjectRegistrationForm').attr('action'),
            data: $('#BmissSubjectRegistrationForm').serialize(),
            beforeSend: function() {
              $("#global-loader").css('display','block');
               $(".page-loader").css({'position':'relative','top':'50%','left':'50%'});
            },
            success: function(response) {
              ...
              if(response.success){
                ...
                $('#BmissSubjectRegistrationForm')[0].reset();            
  ...
  $('#update_bmiss_form').click(function(e) {
      $("#global-loader").css('display','block');
      $(".page-loader").css({'position':'relative','top':'50%','left':'50%'});
		var error = 0;
        $('#exam_center_error').text('');
      ...
      var totalSelectedSenatePaperSemOne = $('.senateSemOne:checked').length;
        ...
        $.ajax({
            type: 'POST',
            url: $('#ModifyBmissSubjectRegistrationForm').attr('action'),
            data: $('#ModifyBmissSubjectRegistrationForm').serialize(),
            beforeSend: function() {
              $("#global-loader").css('display','block');
               $(".page-loader").css({'position':'relative','top':'50%','left':'50%'});
            },
            success: function(response) {
              ...
              if(response.success){
                $("#message_alert").html(ValidationErrorAlert("<strong>Success! </strong> "+response.message))
                $('#ModifyBmissSubjectRegistrationForm')[0].reset();            
                showSuccessAlertBox(response.message,response.redirect_url)
              }else{
