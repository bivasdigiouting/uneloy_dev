...
(function () {
	...
	function init() {
		...
		$('.sidebar-menu a.active').each(function () {
			var $activeLink = $(this);
			var $submenuParents = $activeLink.parents('li.submenu');
			$($submenuParents.get().reverse()).each(function () {
				var $submenu = $(this);
				var $submenuToggle = $submenu.children('a:first');
				if (!$submenuToggle.hasClass('active')) {
					$submenuToggle.addClass('active');
				}
				if (!$submenuToggle.hasClass('subdrop')) {
					$submenuToggle.addClass('subdrop');
				}
				$submenuToggle.next('ul').show();
			});
		});
	}
	...
	$(document).on('mouseover', function(e) {
        ...
        if ($('body').hasClass('mini-sidebar') && $('#toggle_btn').is(':visible')) {
            ...
            if (targ) {
                ...
                // Comment out the line below to prevent automatic closing of submenus
                // $('.subdrop + ul').slideUp();
            }
            return false;
        }
	...
	function colinit() {
		...
		$('.sidebar-right a.active').each(function () {
			var $activeLink = $(this);
			var $submenuParents = $activeLink.parents('li.submenu');
			$($submenuParents.get().reverse()).each(function () {
				var $submenu = $(this);
