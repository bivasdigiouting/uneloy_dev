                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="profile-img-wrap edit-img">
                                        @if($user->image)
                                            <img class="inline-block" src="{{ Storage::url($user->image) }}" alt="user" id="imagePreview">
                                        @else
                                        ...
                                        @if($user->image)
                                            <button type="button" class="btn btn-danger btn-sm" onclick="removeImage({{ $user->id }})">Remove Image</button>
                                        @endif
                                    </div>
                                    @error('image')
...
<script>
    ...
    function removeImage(userId) {
        if (confirm('Are you sure you want to remove this image?')) {
            fetch(`{{ url('admin/users') }}/${userId}/image`, {
                method: 'DELETE',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    document.getElementById('imagePreview').src = '{{ asset('backend-assets/img/profiles/avatar-02.jpg') }}';
                    location.reload();
                } else {
