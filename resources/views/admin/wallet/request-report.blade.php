...
@section('title', 'Level Wallet Req. Report')
...
<div class="content">
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
        <div class="my-auto mb-2">
            <h2 class="mb-1">Level Wallet Req. Report</h2>
            <nav>
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Level Wallet Req. Report</li>
                </ol>
            </nav>
        </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="walletRequestsTable" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Action</th>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const table = $('#walletRequestsTable').DataTable({
            processing: true,
            serverSide: true,
            searching: false,
            order: [[8, 'desc']],
            ajax: {
                url: '{{ route('admin.wallet.request-report.data') }}',
                data: function (d) {
        ...
        $('#walletRequestsTable').on('xhr.dt', function (e, settings, json, xhr) {
            if (json && json.totals) {
        ...
        $('#walletRequestsTable').on('click', '.js-wr-view', async function () {
        ...
        $('#walletRequestsTable').on('click', '.js-wr-approve', async function () {
            const id = this.getAttribute('data-id');
            const source = this.getAttribute('data-source');
            try {
        ...
        $('#walletRequestsTable').on('click', '.js-wr-reject', async function () {
            const id = this.getAttribute('data-id');
            const source = this.getAttribute('data-source');
            try {
