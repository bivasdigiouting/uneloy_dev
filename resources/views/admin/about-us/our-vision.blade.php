            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col">
                            <h4 class="card-title mb-0">Edit Our Vision</h4>
                        </div>
                    </div>
                </div>
                <form action="{{ route('admin.about-us.our-vision.update') }}" method="POST" enctype="multipart/form-data" id="ourVisionForm">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-8">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control @error('text_header') is-invalid @enderror" id="text_header" name="text_header" value="{{ old('text_header', $ourVision->text_header ?? '') }}" maxlength="255">
                                    @error('text_header')
                                        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea class="form-control @error('text_description') is-invalid @enderror" id="text_description" name="text_description" rows="6">{{ old('text_description', $ourVision->text_description ?? '') }}</textarea>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea class="form-control @error('footer_short_description') is-invalid @enderror" id="footer_short_description" name="footer_short_description" rows="4">{{ old('footer_short_description', $ourVision->footer_short_description ?? '') }}</textarea>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="image-preview-container">
                                    @if($ourVision && $ourVision->image)
                                        <img src="{{ $ourVision->image_url }}" alt="Current Image" class="img-fluid rounded border" style="max-height: 200px; width: 100%; object-fit: cover;">
                                    @else
                                ...
                                @if($ourVision)
                                    <div class="mt-3">
                                        <small class="text-muted"><strong>Created:</strong> {{ $ourVision->created_at ? $ourVision->created_at->format('M d, Y H:i') : 'N/A' }}<br><strong>Updated:</strong> {{ $ourVision->updated_at ? $ourVision->updated_at->format('M d, Y H:i') : 'N/A' }}</small>
                                    </div>
                                @endif
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group text-end">
                                    <button type="submit" class="btn btn-primary"><i class="ti ti-device-floppy me-1"></i>Update Our Vision</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    $('#ourVisionForm').on('submit', function(e) {
        e.preventDefault();
        Swal.fire({
            title: 'Update Our Vision?',
