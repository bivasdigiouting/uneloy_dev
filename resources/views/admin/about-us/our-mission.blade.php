            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col">
                            <h4 class="card-title mb-0">Edit Our Mission</h4>
                        </div>
                    </div>
                </div>
                <form action="{{ route('admin.about-us.our-mission.update') }}" method="POST" enctype="multipart/form-data" id="ourMissionForm">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-8">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control @error('text_header') is-invalid @enderror" id="text_header" name="text_header" value="{{ old('text_header', $ourMission->text_header ?? '') }}" maxlength="255">
                                    @error('text_header')
                                        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea class="form-control @error('text_description') is-invalid @enderror" id="text_description" name="text_description" rows="6">{{ old('text_description', $ourMission->text_description ?? '') }}</textarea>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea class="form-control @error('footer_short_description') is-invalid @enderror" id="footer_short_description" name="footer_short_description" rows="4">{{ old('footer_short_description', $ourMission->footer_short_description ?? '') }}</textarea>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="image-preview-container">
                                    @if($ourMission && $ourMission->image)
                                        <img src="{{ $ourMission->image_url }}" alt="Current Image" class="img-fluid rounded border" style="max-height: 200px; width: 100%; object-fit: cover;">
                                    @else
                                ...
                                @if($ourMission)
                                    <div class="mt-3">
                                        <small class="text-muted"><strong>Created:</strong> {{ $ourMission->created_at ? $ourMission->created_at->format('M d, Y H:i') : 'N/A' }}<br><strong>Updated:</strong> {{ $ourMission->updated_at ? $ourMission->updated_at->format('M d, Y H:i') : 'N/A' }}</small>
                                    </div>
                                @endif
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group text-end">
                                    <button type="submit" class="btn btn-primary"><i class="ti ti-device-floppy me-1"></i>Update Our Mission</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    $('#ourMissionForm').on('submit', function(e) {
        e.preventDefault();
        Swal.fire({
