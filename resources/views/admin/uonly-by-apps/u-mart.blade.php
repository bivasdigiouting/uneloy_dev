...
@section('title', 'Uonly By Apps - U-Mart')

@section('content')
<div class="content">
    <div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
        <div class="my-auto mb-2">
            <h2 class="mb-1">Uonly By Apps - U-Mart</h2>
            <nav>
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">U-Mart</li>
                </ol>
            </nav>
        </div>
    </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col">
                            <h4 class="card-title mb-0">Edit U-Mart</h4>
                        </div>
                    </div>
                </div>
                <form action="{{ route('website-uonly-by-apps.u-mart.update') }}" method="POST" enctype="multipart/form-data" id="uonlyByAppsUMartForm">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-8">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control @error('text_header') is-invalid @enderror" id="text_header" name="text_header" value="{{ old('text_header', $uMart->text_header ?? '') }}" maxlength="255">
                                    @error('text_header')
                                        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea class="form-control @error('text_description') is-invalid @enderror" id="text_description" name="text_description" rows="6">{{ old('text_description', $uMart->text_description ?? '') }}</textarea>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea class="form-control @error('footer_short_description') is-invalid @enderror" id="footer_short_description" name="footer_short_description" rows="4">{{ old('footer_short_description', $uMart->footer_short_description ?? '') }}</textarea>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="image-preview-container">
                                    @if($uMart && $uMart->image)
                                        <img src="{{ $uMart->image_url }}" alt="Current Image" class="img-fluid rounded border" style="max-height: 200px; width: 100%; object-fit: cover;">
                                    @else
                                ...
                                @if($uMart)
                                    <div class="mt-3">
                                        <small class="text-muted"><strong>Created:</strong> {{ $uMart->created_at ? $uMart->created_at->format('M d, Y H:i') : 'N/A' }}<br><strong>Updated:</strong> {{ $uMart->updated_at ? $uMart->updated_at->format('M d, Y H:i') : 'N/A' }}</small>
                                    </div>
                                @endif
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group text-end">
                                    <button type="submit" class="btn btn-primary"><i class="ti ti-device-floppy me-1"></i>Update U-Mart</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
    $('#text_description').summernote({
        height: 200,
        placeholder: 'Enter U-Mart description...',
        toolbar: [
            ['fontsize', ['fontsize']],
    ...
    $('#uonlyByAppsUMartForm').on('submit', function(e) {
        e.preventDefault();
        Swal.fire({
            title: 'Update U-Mart?',
