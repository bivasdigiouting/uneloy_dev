...
@section('title','Set E-Card Permissions')
...
<div class="content container-fluid">
    <div class="page-header">
        <div class="row align-items-center">
            <div class="col">
                <h3 class="page-title">Set E-Card Permissions</h3>
            </div>
            <div class="col-auto">
                <form method="post" action="{{ route('admin.ecard-permissions.sync') }}">
                    @csrf
                    <button class="btn btn-warning">Sync From E-Card Menu</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            <form method="get" action="{{ route('admin.ecard-permissions.index') }}" class="row g-3 mb-4">
                <div class="col-md-4">
                    <label class="form-label">Department Level</label>
                    <select name="department" class="form-select" required>
                        <option value="">Select</option>
                        @foreach(['State e-Card Seva','District e-Card Seva','Block - e-Card Seva','G P M e-Card Seva','e-Card Seva','Member'] as $dl)
                            <option value="{{ $dl }}" {{ $department===$dl?'selected':'' }}>{{ $dl }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            @if($department)
            <form method="post" action="{{ route('admin.ecard-permissions.save') }}">
                @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.module-block tbody tr').forEach(function(row) {
        ...
        // Individual checkbox changes update row/module toggles
    ...
    // Keep global in sync on any checkbox change
    document.querySelectorAll('.perm-checkbox').forEach(function(cb) {
        cb.addEventListener('change', updateGlobalSelectAll);
    });
