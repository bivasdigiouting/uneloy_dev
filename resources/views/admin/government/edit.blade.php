...
@section('title', 'Government Section')

@section('content')
<div class="content">
    <div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
        <div class="my-auto mb-2">
            <h2 class="mb-1">Government Section</h2>
            <nav>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col">
                            <h4 class="card-title mb-0">Edit Government Section</h4>
                        </div>
                    </div>
                </div>
                <form action="{{ route('admin.government.update') }}" method="POST" enctype="multipart/form-data" id="governmentProfileForm">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-8">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control @error('text_header') is-invalid @enderror" id="text_header" name="text_header" value="{{ old('text_header', $government->text_header ?? '') }}" maxlength="255">
                                    @error('text_header')
                                        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea class="form-control @error('text_description') is-invalid @enderror" id="text_description" name="text_description" rows="6">{{ old('text_description', $government->text_description ?? '') }}</textarea>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea class="form-control @error('footer_short_description') is-invalid @enderror" id="footer_short_description" name="footer_short_description" rows="4">{{ old('footer_short_description', $government->footer_short_description ?? '') }}</textarea>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="image-preview-container">
                                    @if($government && $government->image)
                                        <img src="{{ $government->image_url }}" alt="Current Image" class="img-fluid rounded border" style="max-height: 200px; width: 100%; object-fit: cover;">
                                    @else
                                ...
                                @if($government)
                                    <div class="mt-3">
                                        <small class="text-muted"><strong>Created:</strong> {{ $government->created_at ? $government->created_at->format('M d, Y H:i') : 'N/A' }}<br><strong>Updated:</strong> {{ $government->updated_at ? $government->updated_at->format('M d, Y H:i') : 'N/A' }}</small>
                                    </div>
                                @endif
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group text-end">
                                    <button type="submit" class="btn btn-primary"><i class="ti ti-device-floppy me-1"></i>Update Government Section</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
    $('#text_description').summernote({
        height: 200,
        placeholder: 'Enter government section description...',
        toolbar: [
            ['fontsize', ['fontsize']],
    ...
    $('#governmentProfileForm').on('submit', function(e) {
        e.preventDefault();
        Swal.fire({
            title: 'Update Government Section?',
