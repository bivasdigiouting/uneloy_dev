...
@section('title', 'Panchayat Master')

@section('content')
<div class="content">
    <div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
        <div class="my-auto mb-2">
            <h2 class="mb-1">Panchayat Management</h2>
            <nav>
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="ti ti-smart-home"></i></a></li>
                    <li class="breadcrumb-item">Master Modules</li>
                    <li class="breadcrumb-item active" aria-current="page">Panchayat Master</li>
                </ol>
            </nav>
        </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col"><h4 class="card-title mb-0">Panchayats List</h4></div>
                        <div class="col-auto"><a href="{{ route('admin.panchayats.create') }}" class="btn btn-primary"><i class="ti ti-plus me-1"></i> Add New Panchayat</a></div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="panchayatsTable" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
                            <thead>
                                <tr>
                                    <th>S.No</th>
                                    <th>Panchayat</th>
...
<script>
$(function(){
    var table = $('#panchayatsTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: { url: "{{ route('admin.panchayats.index') }}", type: 'GET' },
        columns: [
    ...
    $(document).on('click','.toggle-status',function(){
        ...
        Swal.fire({ title:'Are you sure?', icon:'warning', showCancelButton:true }).then((r)=>{
            if(r.isConfirmed){
                $.post("{{ url('admin/panchayats') }}/"+id+"/toggle-status", { _token: '{{ csrf_token() }}' })
    ...
    $(document).on('click','.delete-row',function(){
        ...
        Swal.fire({ title:'Delete?', text:'This cannot be undone', icon:'warning', showCancelButton:true }).then((r)=>{
            if(r.isConfirmed){
                $.ajax({ url: "{{ url('admin/panchayats') }}/"+id, type:'DELETE', data:{ _token: '{{ csrf_token() }}' } })
