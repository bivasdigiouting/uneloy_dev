...
<head>
    ...
    <title>Benefit - E-Card Seva Other Req. Details</title>
...
<body>
    ...
    <div class="container py-4">
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h2 class="page-title">E-Card Seva Other Req. Details</h2>
        </div>
                        ...
                        <tbody>
                            @foreach(($others ?? []) as $row)
                                <tr>
                                    ...
                                    <td>{{ $row['service'] }}</td>
                                    <td>{{ $row['date'] }}</td>
                                    <td><span class="badge {{ $row['status'] === 'In Progress' ? 'bg-info' : 'bg-warning' }}">{{ $row['status'] }}</span></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
