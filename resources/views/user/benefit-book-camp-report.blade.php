...
<head>
    ...
    <title>Benefit - Book Camp Report</title>
...
<body>
    ...
    <div class="container py-4">
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h2 class="page-title">Book Camp Report</h2>
        </div>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped">
                        <thead><tr><th>Camp</th><th>Date</th><th>Status</th></tr></thead>
                        <tbody>
                            @foreach(($report ?? []) as $row)
                                <tr>
                                    <td>{{ $row['name'] }}</td>
                                    <td>{{ $row['date'] }}</td>
                                    <td><span class="badge {{ $row['status'] === 'Approved' ? 'bg-success' : 'bg-warning' }}">{{ $row['status'] }}</span></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
