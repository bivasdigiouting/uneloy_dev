...
<head>
    ...
    <title>Other Blood Details</title>
    ...
    <style>
        ...
        .list-shell { max-width: 760px; margin: 0 auto; padding: 12px 14px 28px; }
        ...
        .status-pill.pending { background: #f6ddb5; color: #6b4e16; }
        ...
        @media (min-width: 992px) {
            ...
            .list-shell { padding-left: 0; padding-right: 0; }
        }
    </style>
</head>
<body>
    <div class="topbar">
        <div class="topbar-inner">
            ...
            <h1 class="page-title">Other Blood Details</h1>
            <div style="width: 44px;"></div>
        </div>
    </div>

    @if(($otherDonations ?? collect())->count() === 0)
        <div class="page-shell">
            <div class="empty-wrap">
                <svg class="empty-illustration" viewBox="0 0 900 520" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="No data">
                    ...
                    <ellipse cx="330" cy="360" rx="250" ry="130" fill="#efe7ff"/>
    ...
    @else
        <div class="list-shell">
            @foreach(($otherDonations ?? []) as $row)
                <div class="request-card">
                    <div class="d-flex align-items-start justify-content-between gap-3">
                        <div>
                            <p class="name">{{ $row->name ?? 'Donation' }}</p>
                        </div>
                        <div class="status-pill {{ ($row->status ?? '') === 'Pending' ? 'pending' : '' }}">{{ $row->status ?? '-' }}</div>
                    </div>
