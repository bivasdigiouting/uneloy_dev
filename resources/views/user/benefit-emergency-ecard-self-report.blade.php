...
<head>
    ...
    <title>Benefit - Emergency E-Card Self Req. Report</title>
...
<body>
    ...
    <div class="container py-4">
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h2 class="page-title">Emergency E-Card Self Req. Report</h2>
        </div>
                        ...
                        <tbody>
                            @foreach(($emergencyRequests ?? []) as $row)
                                <tr>
                                    <td>{{ $row['issue'] }}</td>
                                    <td>{{ $row['date'] }}</td>
                                    <td><span class="badge {{ $row['status'] === 'Resolved' ? 'bg-success' : 'bg-warning' }}">{{ $row['status'] }}</span></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
