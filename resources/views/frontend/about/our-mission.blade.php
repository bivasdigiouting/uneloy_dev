...
<main class="container my-5" style="min-height: 50vh;">
    @if($ourMission)
        <div class="row align-items-center mb-5 mt-4">
            <div class="col-lg-6 mb-4 mb-lg-0">
                <h1 class="display-6 fw-bold text-primary mb-3">{{ $ourMission->text_header }}</h1>
                <div class="lead text-muted fs-5">
                    {!! $ourMission->text_description !!}
                </div>
            </div>
            <div class="col-lg-6 text-center">
                @if($ourMission->image_url)
                    <img src="{{ $ourMission->image_url }}" alt="{{ $ourMission->text_header }}" class="img-fluid rounded shadow-sm" style="max-height: 400px; object-fit: cover; width: 100%;">
                @else
        ...
        @if($ourMission->footer_short_description)
            <div class="row">
                <div class="col-12 mt-4 p-4 bg-light rounded text-center">
                    <div class="text-secondary fs-5">
                        {!! $ourMission->footer_short_description !!}
                    </div>
                </div>
            </div>
        @endif
    @else
        <div class="text-center py-5 mt-5">
            <h1 class="display-5 fw-bold text-primary">Our Mission</h1>
